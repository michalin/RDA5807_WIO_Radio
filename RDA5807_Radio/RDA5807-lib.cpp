/**
 * @mainpage RDA5807 Arduino Library implementation, Modified
...
void RDA5807::powerUp()
{
    ...
    reg05->refined.LNA_ICSEL_BIT = 3; // Mod: more sensitivity
...
{
    if (frequency > this->endBand[currentFMBand]) // Mod: Ignore if out of upper band limit
    {
        frequency = this->endBand[currentFMBand];
    }
 ...
 * | 11    | 65 –76 MHz (East Europe) or 50-76MHz (see bit 9 of register 0x07) |
 * | 100   | 50 - 76 MHz Mod
...
void RDA5807::setBand(uint8_t band)
{
    ...
    if (band == 4) // Lower limit: 50 MHz
    {
        startBand[3] = 5000;
        reg07->refined.MODE_50_60 = 0;
        band = 3;
    }
    else
    {
        startBand[3] = 6500;
        reg07->refined.MODE_50_60 = 1;
    }
    reg03->refined.BAND = this->currentFMBand = band; // Mod
    setRegister(REG03, reg03->raw);
...
{
    reg03->refined.SPACE = currentFMSpace = space;
    setRegister(REG03, reg03->raw);
}
...
uint32_t RDA5807::rdsTest()
{
    rds_blockb blkb;
    getStatus(REG0B);
    blkb.blockB = reg0d->RDSB;
    uint8_t gtype = blkb.refined.groupType;
    uint8_t version = blkb.refined.versionCode;
    uint8_t addr = blkb.refined.content;
    char status[128];
    uint16_t blockC = reg0e->RDSC;
    char blc[3];
    blc[0] = blockC >> 8;
    blc[1] = blockC;
    uint16_t blockD = reg0f->RDSD;
    char bld[3];
    bld[0] = blockD >> 8;
    bld[1] = blockD;
    ...
    // if(gtype == 1)
    {
        sprintf(status, "group: %d, version: %d, addr: %02x, CD: %s %s ", gtype, version, addr, blc, bld);
